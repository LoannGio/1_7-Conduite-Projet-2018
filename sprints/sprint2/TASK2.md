...
|---|---|---|---|---|---|---|---|
|T#2.1D|-|Dans le fichier "Definitions.md" définir ce qu'est un "Développeur" (utilisateur connecté à notre application et objet de notre BD, il faut définir ses attributs (obligatoires et facultatifs)).|0.1|-|-|DONE|CarlosVilla|
|T#2.2D|-|Dans le fichier "Definitions.md" définir ce qu'est une "User Story" (objet de notre BD, il faut définir ses attributs(obligatoires et facultatifs)).|0.1|-|-|DONE|LoannGio|
|T#2.3D|-|Dans le fichier "Definitions.md" définir ce qu'est une "Sprint" (objet de notre BD, il faut définir ses attributs(obligatoires et facultatifs)).|0.1|-|-|DONE|LoannGio|
|T#2.4D|-|Dans le fichier "Definitions.md" définir les routes et les fichiers qui vont permettre la gestion des utilisateurs, user stories, sprints, projets.|0.25|-|-|DONE|LoannGio|
|T#2.5D|views/createAccount.ejs|Créer le fichier "createAccount.ejs" qui affiche la page permettant à l'utilisateur de créer son compte en entrant son addresse mail, son nom d'utilisateur et son mot passe.|0.1|-|US#1|DONE|Aliénor Brabant|
|T#2.6D|views/project.ejs|Créer le fichier "project.ejs" qui affiche la page permettant aux developeurs de visualiser le projet: nom du projet, description, durée d'un sprint, backlog (liste des US), liste des sprints.|0.1|-|US#9|DONE|Aliénor Brabant|
|T#2.7D|views/projectModify.ejs|Créer le fichier "projectModify.ejs" qui affiche la page permettant de modifier un projet: description, nom, durée d'un sprint.|0.1|-|US#4|DONE|Aliénor Brabant|
...
|T#2.10D|model/login.js|Créer le fichier "login.js" dont le rôle est de communiquer avec la base de données afin de gérer les comptes. (création d'un utilisateur, vérification de l'existance de l'utilisateur) "addUser(user)" pour ajouter un utilisateur. "existUser(user)" pour vérifier si un utilisateurs exist déjà dans la BD.|0.1|-|US#4, US#8, US#9|DONE|Aliénor Brabant|
|T#2.11D|model/project.js|Créer le fichier "project.js" dont le rôle est de communiquer avec la base de données afin de gérer un projet (modification des attributs, ajout/suppression/modification du backlog, ajout de sprint). "updateProject(projectAttribut)" pour modifier les attribut d'un projet. "getProject(id)" pour obtenir le projet correspondant à l'id.|0.1|-|US#12|DONE|CarlosVilla|
...
|T#2.21T|-|Implementation de test(s) unitaire(s) pour le module "model/login.js"|0.25|T#2.1D, T#2.10D|US#1|DONE|LoannGio|
|T#2.22T|-|Implementation de test(s) unitaire(s) pour le module "model/project.js"|0.25|T#2.2D, T#2.11D|US#4, US#8, US#9|ON GOING|LoannGio|
|T#2.23T|-|Implementation de test(s) unitaire(s) pour le module "model/sprint.js"|0.25|T#2.3D, T#2.12D|US#11, US#12|TODO|-|
|T#2.24E|-|Ecriture du test de validation de US#1.|0.1|-|US#1|DONE|LoannGio|
|T#2.25E|-|Ecriture du test de validation de US#4.|0.1|-|US#4|DONE|CarlosVilla|
|T#2.26E|-|Ecriture du test de validation de US#8.|0.1|-|US#8|ON GOING|CarlosVilla|
...
|T#2.30T|-|Réaliser le test de validation de US#1.|0.25|T#2.13I, T#2.21T, T#2.24E|US#2|DONE|CarlosVilla|
|T#2.31T|-|Réaliser le test de validation de US#4.|0.25|T#2.15I, T#2.22T, T#2.25E|US#4|DONE|CarlosVilla|
